...
D3D CVV(Canonical View Volume)的z範圍為[0,1]
...
![](image\PerspectiveFrustumAndNdc.png)
...
![](image\PerspectiveFrustumTopView.png) ![](image\PerspectiveFrustumSideView.png)
...
最後P點除以自身的 Wp 變成Normalized Device Coordinates中的一點N

從P到N的除以Wp運算中，我們發現這是個線性關係，可以用**直線方程式**來定義 y=mx+b

![](image\y-mc-b.bmp)

N(Xn, Yn, Zn) = (Xp/Wp, Yp/Wp, Zp/Wp)

用矩陣表示如下

    [Xp]	[					]	[ Xe ]
    [Yp] =	[ Projection Matrix	] x	[ Ye ]
    [Zp]	[					]	[ Ze ]
    [Wp]	[					]	[ We ]
...
7. Xn=Xp/Wp,Wp=-Ze, 所以 Xp=2n/(r-l)*Xe + (r+l)/(r-l)*Ze 
...
7. Yn=Yp/Wp,Wp=-Ze, 所以 Yp=2n/(t-b)*Ye + (t+b)/(t-b)*Ze
 
矩陣如下

    [Xp]	[2n/(r-l)	0			(r+l)/(r-l)		0	]	[ Xe ]
    [Yp] =	[0			2n/(t-b)	(t+b)/(t-b)		0	] x	[ Ye ]
    [Zp]	[.			.			.				.	]	[ Ze ]
    [Wp]	[0			0			-1				0	]	[ We ]


求 Zp ，我們已知Zp不會被 Xe 跟 Ye所影響，然後我們設剩下的兩個為代數A,B , 然後E點是一個點，所以We=1
...
###OpenGL CVV(Canonical View Volume)的z範圍為[-1,1]
